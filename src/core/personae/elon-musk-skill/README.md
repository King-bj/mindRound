...
本仓库的 [`SKILL.md`](SKILL.md) frontmatter 含 `name`、`displayName` 等字段。**宿主应用展示人物名称时**，建议采用与 SKILL 内 **「元数据与界面显示」** 相同的解析顺序：

**`displayName`（若存在且非空）→ `name` → 目录名（id）**

列表、会话标题、圆桌标签等均应使用上述「显示名」，避免只绑定 `name` 导致用户看到内部标识符。
